r"""Geometric Gradient Rectification (GGR) step for ``unimatch_v2_1gpu.py``.

Chen et al., "Geometric Gradient Rectification for Safe Open-Set SSL". The
supervised gradient is the anchor and is never modified; only the unsupervised
part is projected, and the two are then re-summed into ``.grad``. The caller
runs this after its raw-conflict diagnostics and before ``optimizer.step()``,
so ``grad/grad_cos_x_s`` keeps its meaning as the *pre*-rectification conflict
metric (paper section C.1) while ``grad/ggr_cos_after`` is the
post-rectification (applied) one.

Modes (``args.ggr``), all restricted to the surgery scope
$[\mathrm{scope\_lo}, \mathrm{scope\_hi})$ of the flat gradient buffers:

* ``vlr``: Eq. (5), remove the component of $g_u$ along $-g_s$ (or along the
  $1/f$-weighted class anchor $g_w$ under ``--ggr-anchor class``).
* ``osr``: Eq. (7), remove the whole component of $g_u$ inside
  $\mathrm{span}(U)$; $U$ is a ring buffer of recent anchors (Alg. 1) or the
  Gram-Schmidt basis of the class prototypes.
* ``csr``: Eq. (8), clip only the coordinates of $g_u$ that oppose a basis row.
* ``cone``: exact projection of $g_u$ onto the foreground cone
  $\{g : \langle g, G_c \rangle \ge 0\}$ (``util/csr_cone.py``).

This module holds only the per-step rectification; the buffers it works on
(``U``, ``proto``, the flat gradient views, the scope indices) are set up and
owned by the training script.
"""
import torch
...
def rectify(args, *, iters, flat_x, flat_s, scope_lo, scope_hi, scope_p_lo, scope_p_hi,
            params, grad_x_acc, grad_s_acc, scope_norm_x_sq, scope_norm_s_sq, scope_dot_xs,
            class_anchor, seen, n_seen, proto, proto_w, proto_order,
            U, u_k, u_ptr, u_iters, ggr_scratch, class_names, logger):
    r"""Rectify the unsupervised gradient in place and reassemble ``.grad`` on the scope.

    Returns ``(ggr_stats, u_k, u_ptr)``: the wandb ``grad/ggr_*`` diagnostics for
    this step (empty under ``--ggr none``) and the updated ring-buffer counters.
    ``flat_s`` (through its scope slice), the scope's ``p.grad``, ``U`` and
    ``u_iters`` are modified in place; ``flat_x`` and ``proto`` are read only.

    Arguments mirror the training-loop variables of the same name:

    * ``args``: reads ``ggr``, ``ggr_cone_rescale`` and ``ggr_reorth_every``.
    * ``flat_x`` / ``flat_s``: flat supervised / unsupervised gradient buffers;
      ``grad_x_acc`` / ``grad_s_acc`` are their per-parameter views over ``params``.
    * ``scope_lo``, ``scope_hi``: the surgery scope as a slice of the flat
      buffers; ``scope_p_lo``, ``scope_p_hi`` the matching slice of ``params``.
    * ``scope_norm_x_sq``, ``scope_norm_s_sq``, ``scope_dot_xs``: the scope's
      $\|g_s\|^2$, $\|g_u\|^2$ and $\langle g_s, g_u \rangle$ before rectification.
    * ``class_anchor``: ``--ggr-anchor class``; then ``seen`` (bool per class),
      ``n_seen``, ``proto`` (class prototypes laid out like the scope),
      ``proto_w`` ($1/f$ weights) and ``proto_order`` (rarest class first) are set.
    * ``U`` (``(d, D_scope)``, orthonormal rows), ``u_k`` filled rows, ``u_ptr``
      next ring slot, ``u_iters`` per-row write step, ``ggr_scratch`` a
      ``D_scope`` work buffer: the recent-anchor subspace state (``osr``/``csr``).
    * ``class_names``: ``CLASSES[cfg['dataset']]``, for the per-class stat keys.
    """
    ggr_stats = {}
    if args.ggr == 'none':
        return ggr_stats, u_k, u_ptr
    ...
    if args.ggr == 'vlr' and g_w is not None:
        ...
        if class_anchor:
            ...
            for i, cidx in enumerate([int(c) for c in proto_order.tolist() if seen[c]][:u_k]):
                ggr_stats['grad/ggr_coef/%s' % class_names[cidx]] = coef[i].item()
        if args.ggr == 'csr':
